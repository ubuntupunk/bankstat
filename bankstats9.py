...
def parse_transactions(content):
    ...
    for match in matches:
        ...
        try:
            amounts = [Decimal(a.replace(',', '')) for a in [amount1, amount2] if a]
            if len(amounts) == 1:
                transactions.append((date, description.strip(), amounts[0], Decimal('0')))
            elif len(amounts) == 2:
                transactions.append((date, description.strip(), amounts[0], amounts[1]))
